...
def panoptic_segmentation(cfg, outputs, device):
    """
    ...
    if outputs['instance'] is None or outputs['instance'] == dict():
    ...
    for i, instance in enumerate(outputs['instance']):
        ...
        # print(semantic_stuff_logits.shape)
        # print(inter_logits.shape)
        # print(inter_preds)
        # import numpy as np
        # print(np.unique(inter_preds.cpu()))
        # import matplotlib.pyplot as plt

        # def visualize_mask(mask):
        #     plt.figure(figsize=(6, 6))
        #     plt.imshow(mask.cpu().numpy(), cmap='gray')  # Convert to NumPy and display
        #     plt.axis('off')  # Hide axes
        #     plt.title("Panoptic Segmentation Mask")
        #     plt.show()

        # # Example Usage (assuming inter_preds is a PyTorch tensor)
        # visualize_mask(inter_preds)
...
def create_mlb(semantic, instance):
    ...
    for bbox, cls in zip(boxes, classes):
        ...
        canva = torch.ones(instance.image_size)
...
def create_canvas_thing(inter_preds, instance):
    ...
    classes = instance.pred_classes
    # print(classes)
    instance_train_id_to_eval_id = [2, 0]
...
def compute_output_only_semantic(semantic):
    ...
    """
    semantic_train_id_to_eval_id = [1, 2, 0]
    ...
    # import numpy as np
    # print(np.unique(semantic_output).cpu())
