...
class CustomROIHeads(ROIHeads):
    ...
    def forward(
        ...
        if self.training:
            ...
            # Only compute mask loss if there are proposals
            if len(proposals) > 0 and any(len(p) > 0 for p in proposals):
                mask_losses = self._forward_mask(features, proposals)
                if mask_losses:  # Ensure it is not None
...
class MaskNetwork(BaseMaskRCNNHead):
    ...
    def __init__(self, num_classes):
        ...
        # Instead of self.deconv 
        # self.deconv = nn.Sequential(
        # nn.Conv2d(256, 1024, kernel_size=3, padding=1),
        # nn.PixelShuffle(2),  # Upsamples from 28x28 to 56x56
        # nn.Conv2d(256, 256, kernel_size=3, padding=1),
        # nn.PixelShuffle(2),  # Upsamples from 56x56 to 112x112
        # nn.Conv2d(64, 64, kernel_size=3, padding=1),
        # nn.PixelShuffle(2),  # Upsamples from 112x112 to 224x224
        # nn.Conv2d(16, 16, kernel_size=3, padding=1),
        # nn.PixelShuffle(2),  # Upsamples from 224x224 to 256x256
        # )
