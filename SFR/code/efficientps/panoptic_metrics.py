...
def generate_pred_panoptic(cfg, outputs):
    ...
    mask_folder_path = "/mnt/e3dbc9b9-6856-470d-84b1-ff55921cd906/EPS_Medical/Liver_dataset/preds_masks_only"
    ...
    for output in tqdm(outputs):
        ...
        for img_panoptic, image_id in zip(output['panoptic'], output['image_id']):
            ...
            mask_path = os.path.join(mask_folder_path, "{}_mask_preds_panoptic.png".format(image_id))
            img_mask = np.where(img_panoptic == 1, 0, 255).astype(np.uint8)
            img_mask_to_save = Image.fromarray(img_mask)
            img_mask_to_save.save(mask_path)
