...
def Spread_bs_aug(image, label, input_size, nt=0, nonlinear_rate=0.5, rdmrotflip=False, rdmcrop_2d=False, flip_2d=False, shuffle_2d=False):        # 每一张slice做bezier灰度augmentation
    ...
    for i in range(image.shape[0]):
        ...
        for j in range(single_image.shape[0]):
            ...
            # single_image[j] = torch.from_numpy(slice).float()
            # single_label[j] = torch.from_numpy(slice_lbl).long()  # or .float() depending on the original label type
...
def Spread_bs_aug_scale(image, label, input_size, scale, nt=0, nonlinear_rate=0.5):        # 每一张slice做bezier灰度augmentation
    ...
    for i in range(image.shape[0]):
        single_image, single_label = image[i].squeeze(), label[i].squeeze()
        # print(single_image.shape, single_label.shape)
        single_image, single_label = np.swapaxes(single_image, 0, 2), np.swapaxes(single_label, 0, 2)  # (64, 120, 120)
        # print(single_image.shape, single_label.shape)
...
def ToTensor_sam_bs(image, label):     # [B, 1024, 1024]
    ...
    label_batch_inshape = label_batch_inshape.reshape(label_batch_inshape.shape[0], label_batch_inshape.shape[1], volume_batch_inshape.shape[2]).astype(np.int64)  # [B, 1024, 1024]
...
def sam_test_tensor(model_sam, image, label, input_size, batch_size, labeled_bs, num_classes, multimask_output, nolora=False):
    ...
    with torch.no_grad():
        if nolora:
            ...
            outputs, loss = model_sam.shared_step(batch)
            low_res_masks = outputs['semantic']
        ...
        # # Assume prediction_inshape and label_inshape are both shaped [2, 1024, 1024]
        # # Move to CPU and convert to NumPy
        # pred_np = prediction_inshape.cpu().numpy()
        # label_np = label_inshape.cpu().numpy()
        ...
        #     axes[0, i].imshow(label_np[i], cmap='gray')
        #     axes[0, i].set_title(f"GT Slice {i}")
        #     axes[0, i].axis('off')

        #     # Prediction (bottom row)
        #     axes[1, i].imshow(pred_np[i], cmap='gray')
        #     axes[1, i].set_title(f"Pred Slice {i}")
        #     axes[1, i].axis('off')
    ...
    # print(prediction_sam.shape)
    # print(torch.unique(prediction_sam))
    # raise Exception
